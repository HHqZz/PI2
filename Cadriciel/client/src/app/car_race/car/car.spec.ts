...
describe("Car", () => {
    ...
    it("should decelerate when brake is pressed", () => {
        ...
        car.brake();
        car.update(MS_BETWEEN_FRAMES);
        expect(parseFloat(car.speed.length().toFixed(3))).toBeLessThanOrEqual(parseFloat(initialSpeed.toFixed(3)));
    });
    ...
    it("should set a new position", () => {
        car.setPosition(new Vector3(1, 2, 3));
        expect(car.getPosition()).toEqual(new Vector3(1, 2, 3));
    });
});
